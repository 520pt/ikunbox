...
function loginStatusVod() {
    ...
    return {
        ...
        vod_content: full ? '已检测到完整抖音 Cookie。可使用推荐、精选、儿童博主作品等功能。' : '配置方法：进入“配置｜中心” → “抖音配置” → “设置抖音Cookie”，扫码页可用抖音APP扫码确认；如扫码后未自动写入，请用手动/远程输入粘贴完整 Cookie；也可把 Cookie 写入 /sdcard/TVBox/douyin_cookie.txt，只保留一行，不带 Cookie: 前缀，然后重载配置。Cookie 建议至少包含 sessionid/sessionid_ss、s_v_web_id、UIFID、ttwid，最好也带 msToken。当前缺少：' + (missing || '无')
    };
}
function loginHintVod() {
    return {
        ...
        vod_content: '配置方法：进入“配置｜中心” → “抖音配置” → “设置抖音Cookie”，扫码页可用抖音APP扫码确认；如扫码后未自动写入，请用手动/远程输入粘贴完整 Cookie；也可把 Cookie 写入 /sdcard/TVBox/douyin_cookie.txt，只保留一行，不带 Cookie: 前缀，然后重载配置。Cookie 建议包含 sessionid/sessionid_ss、s_v_web_id、UIFID、ttwid、msToken。'
    };
}
