...
var rule = {
    ...
    timeout: 20000,
    play_parse: false,
    class_name: '登录状态&推荐&关注&精选&影视&综艺&二次元&游戏&音乐&体育&美食&旅行&萌宠&亲子&直播',
    class_url: 'login&recommend&follow&featured&film&entertainment&acg&game&music&sport&food&travel&pet&child&live',
    headers: {
    ...
    二级: $js.toString(() => {
        var id = input || '';
        var item = detailByAwemeId(id);
        VOD = item;
    }),

    搜索: $js.toString(() => {
        VODS = douyinList('search', MY_PAGE || 1, KEY || '');
    }),
    ...
    lazy: $js.toString(() => {
        ...
        input = {
            ...
            jx: 0,
            url: url,
            header: JSON.stringify(mediaHeaders(url))
        };
    })
};
...
function loginStatusVod() {
    ...
    return {
        ...
        vod_content: ok ? '已从 TVBox/douyin_cookie.txt 检测到抖音 Cookie。' : '在电脑浏览器登录 douyin.com，复制 Cookie 到 TVBox/douyin_cookie.txt 后重载配置。'
    };
}
function loginHintVod() {
    return {
        ...
        vod_content: '在电脑浏览器登录 douyin.com，复制 Cookie 到 TVBox/douyin_cookie.txt 后重载配置。'
    };
}
...
function detailByAwemeId(id) {
    if (id === 'login_required' || id === 'featured_empty' || id === 'cookie_ok') {
        var ok = id === 'cookie_ok';
